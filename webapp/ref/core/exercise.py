...
class ExerciseManager:
    ...
    def _parse_attr(
        ...
        if not isinstance(yaml_dict[attr_name], expected_type):
            ...
            raise ExerciseConfigError(
                f'Type of attribute "{attr_name}" is {t}, but {expected_type} was expected.'
            )
        ...
        if validators:
            for fn, err_msg in validators:
                if not fn(ret):
                    raise ExerciseConfigError(
                        f"Validation for attribute {attr_name} failed: {err_msg}"
                    )
    ...
    def _parse_general_data(exercise: Exercise, cfg, cfg_folder_path):
        ...
        if deadline:
            ...
            end = ExerciseManager._parse_attr(
                deadline, "end", dict, required=False, default=None
            )
            if not start or not end:
                raise ExerciseConfigError(
                    'Missing "start:" or "end:" in deadline entry!'
                )
            start_date = ExerciseManager._parse_attr(
                start, "date", datetime.date, required=True, default=None
            )
            start_time = ExerciseManager._parse_attr(
                start, "time", datetime.time, required=True, default=None
            )
            end_date = ExerciseManager._parse_attr(
                end, "date", datetime.date, required=True, default=None
            )
    ...
    def _parse_entry_service(exercise: Exercise, cfg):
        ...
        if "entry" not in cfg:
            raise ExerciseConfigError(
                'An exercise must have exactly one "entry" section'
            )
        ...
        files_to_copy = ExerciseManager._parse_attr(
            entry_cfg, "files", list, required=False, default=[]
        )
        assert isinstance(files_to_copy, list)
        for f in files_to_copy:
            if not isinstance(f, str):
                raise ExerciseConfigError(
                    f"files must be a list of strings {files_to_copy}"
                )
        entry.files = files_to_copy

        build_cmd = ExerciseManager._parse_attr(
            entry_cfg, "build-cmd", list, required=False, default=[]
        )
        assert isinstance(build_cmd, list)
        for line in build_cmd:
            if not isinstance(line, str):
                raise ExerciseConfigError(
                    f"build-cmd must be a list of strings! At least one element is of type {type(line)}!"
                )
        entry.build_cmd = build_cmd

        entry.disable_aslr = False
        disable_aslr = ExerciseManager._parse_attr(
            entry_cfg, "disable-aslr", bool, required=False, default=None
        )
        if disable_aslr is not None:
            raise ExerciseConfigError(
                '"disable-aslr" attribute is deprecated, please use "no-randomize" instead'
            )

        entry.no_randomize_files = ExerciseManager._parse_attr(
            entry_cfg, "no-randomize", list, required=False, default=[]
        )
        entry.cmd = ExerciseManager._parse_attr(
            entry_cfg, "cmd", list, required=False, default=["/bin/bash"]
        )
        entry.persistance_container_path = ExerciseManager._parse_attr(
            entry_cfg, "persistance-path", str, required=False, default="/home/user"
        )
        entry.readonly = ExerciseManager._parse_attr(
            entry_cfg, "read-only", bool, required=False, default=False
        )
        entry.allow_internet = ExerciseManager._parse_attr(
            entry_cfg, "allow-internet", bool, required=False, default=False
        )
        ...
        def __check_mem_limit(val, min_mb):
            ...
            if not match:
                raise ExerciseConfigError(
                    'Invalid memory size value! Please use "GiB" or "MiB" as suffix!'
                )
            ...
            if val < min_mb:
                raise ExerciseConfigError(
                    f"Memory limits must be greater or equal to {min_mb} MiB."
                )

            return int(val)

        limits_config = ExerciseManager._parse_attr(
            entry_cfg, "limits", dict, required=False, default=None
        )
        if limits_config:
            ...
            validators += [
                (
                    lambda v: v >= 0,
                    "Value must be greater or equal to zero. Zero disables this limit.",
                )
            ]
            validators += [
                (
                    lambda v: len(str(v).split(".")[1]) < 2,
                    "No more than 2 decimal places are supported.",
                )
            ]
            entry.ressource_limit.cpu_cnt_max = ExerciseManager._parse_attr(
                limits_config,
                "cpu-cnt-max",
                float,
                required=False,
            ...
            entry.ressource_limit.cpu_shares = ExerciseManager._parse_attr(
                limits_config,
                "cpu-shares",
                int,
                required=False,
            ...
            validators = []
            validators += [
                (lambda v: v >= 64, "Value must be greater or equal than 64")
            ]
            entry.ressource_limit.pids_max = ExerciseManager._parse_attr(
                limits_config,
                "pid-cnt-max",
                int,
                required=False,
            ...
            entry.ressource_limit.memory_in_mb = ExerciseManager._parse_attr(
                limits_config, "phys-mem", str, required=False, default=None
            )
            entry.ressource_limit.memory_swap_in_mb = ExerciseManager._parse_attr(
                limits_config, "swap-mem", str, required=False, default=None
            )
            entry.ressource_limit.memory_kernel_in_mb = ExerciseManager._parse_attr(
                limits_config, "kernel-mem", str, required=False, default=None
            )

            entry.ressource_limit.memory_in_mb = __check_mem_limit(
                entry.ressource_limit.memory_in_mb, 64
            )
            entry.ressource_limit.memory_swap_in_mb = __check_mem_limit(
                entry.ressource_limit.memory_swap_in_mb, 0
            )
            entry.ressource_limit.memory_kernel_in_mb = __check_mem_limit(
                entry.ressource_limit.memory_kernel_in_mb, 64
            )
        ...
        if flag_config:
            entry.flag_path = ExerciseManager._parse_attr(
                flag_config, "location", str, required=False, default="/home/user/flag"
            )
            entry.flag_value = ExerciseManager._parse_attr(
                flag_config, "value", str, required=True
            )
            entry.flag_user = ExerciseManager._parse_attr(
                flag_config, "user", str, required=False, default="admin"
            )
            entry.flag_group = ExerciseManager._parse_attr(
                flag_config, "group", str, required=False, default="admin"
            )
            entry.flag_permission = ExerciseManager._parse_attr(
                flag_config, "permission", int, required=False, default="400"
            )
            del entry_cfg["flag"]

        if entry.readonly and entry.persistance_container_path:
            raise ExerciseConfigError(
                "persistance-path and readonly are mutually exclusive"
            )
    ...
    def _parse_peripheral_services(exercise: Exercise, cfg):
        ...
        for service_name, service_values in peripheral_cfg.items():
            ...
            if not re.fullmatch(service_name_regex, service_name):
                raise ExerciseConfigError(
                    f'Service name "{service_name}"" is invalid ({service_name_regex})'
                )
            service.name = service_name

            if service_name in services_names:
                raise ExerciseConfigError(
                    f"There is already a service with name {service_name}."
                )
            services_names.add(service_name)

            service.disable_aslr = False
            disable_aslr = ExerciseManager._parse_attr(
                service_values, "disable-aslr", bool, required=False, default=None
            )
            if disable_aslr is not None:
                raise ExerciseConfigError(
                    '"disable-aslr" attribute is deprecated, and "no-randomize" ist not implemented for peripheral services yet. Please remove the attribute.'
                )

            service.files = ExerciseManager._parse_attr(
                service_values, "files", list, required=False, default=None
            )
            if service.files:
                for f in service.files:
                    if not isinstance(f, str):
                        raise ExerciseConfigError(
                            f"Files must be a list of strings {service.files}"
                        )

            service.build_cmd = ExerciseManager._parse_attr(
                service_values, "build-cmd", list, required=False, default=None
            )
            if service.build_cmd:
                for line in service.build_cmd:
                    if not isinstance(line, str):
                        raise ExerciseConfigError(
                            f"Command must be a list of strings: {service.build_cmd}"
                        )

            service.cmd = ExerciseManager._parse_attr(service_values, "cmd", list)

            service.readonly = ExerciseManager._parse_attr(
                service_values, "read-only", bool, required=False, default=False
            )

            service.allow_internet = ExerciseManager._parse_attr(
                service_values, "allow-internet", bool, required=False, default=False
            )

            flag_config = service_values.get("flag")
            if flag_config:
                service.flag_path = ExerciseManager._parse_attr(
                    flag_config, "location", str, required=True
                )
                service.flag_value = ExerciseManager._parse_attr(
                    flag_config, "value", str, required=True
                )
                service.flag_user = ExerciseManager._parse_attr(
                    flag_config, "user", str, required=False, default="admin"
                )
                service.flag_group = ExerciseManager._parse_attr(
                    flag_config, "group", str, required=False, default="admin"
                )
                service.flag_permission = ExerciseManager._parse_attr(
                    flag_config, "permission", int, required=False, default="400"
                )
    ...
    def check_global_constraints(exercise: Exercise):
        ...
        for e in predecessors:
            ...
            if (
                e.entry_service.persistance_container_path
                != exercise.entry_service.persistance_container_path
            ):
                raise ExerciseConfigError(
                    "Persistance path changes are not allowed between versions"
                )
    ...
    def create(exercise: Exercise) -> "ExerciseManager":
        ...
        template_path = template_path.joinpath(
            f"{exercise.short_name}-{exercise.version}"
        )
        log.info(f"Creating {template_path}")
        assert not template_path.exists()

        persistence_path = Path(current_app.config["PERSISTANCE_PATH"])
        persistence_path = persistence_path.joinpath(
            f"{exercise.short_name}-{exercise.version}"
        )
        ...
        try:
            ...
            subprocess.run(
                [
                    "/usr/bin/rsync",
                    "-a",
                    f"{exercise.template_import_path}/",
