    ...
    def _parse_entry_service(exercise: Exercise, cfg):
        ...
        for line in build_cmd:
            if not isinstance(line, str):
                raise ExerciseConfigError(f"build-cmd must be a list of strings! At least one element is of type {type(line)}!")
        ...
        entry.persistance_container_path = ExerciseManager._parse_attr(entry_cfg, 'persistance-path', str, required=False, default="/home/user")
