...
class InstanceManager:
    ...
    def create_instance(user: User, exercise: Exercise) -> Instance:
        ...
        try:
            ...
            with inconsistency_on_error(
                f"Error while aborting instance creation {instance}"
            ):
                delete_dirs()
            raise
    ...
    def create_submission(self, test_results: List[SubmissionTestResult]) -> Instance:
        ...
        Raises:
            *: If the instance submission failed.
            InconsistentStateError: If the instance submission failed and left the system
                in an inconsistent state.
        """
        assert not self.instance.submission, (
            f"Can not submit instance {self.instance}, cause it is already part of a submission"
        )
        ...
        try:
            ...
            log.error(
                "Error while coping submitted data into new instance.", exc_info=True
            )
            with inconsistency_on_error():
                new_mgr.remove()
                container.unpause()
    ...
    def update_instance(self, new_exercise: Exercise) -> Instance:
        ...
        try:
            ...
            if (
                not new_exercise.entry_service.readonly
                and self.instance.exercise.entry_service.persistance_container_path
            ):
    ...
    def get_entry_ip(self):
        ...
        container = self.dc.container(self.instance.entry_service.container_id)
        log.info(
            f"Getting IP of container {self.instance.entry_service.container_id} on network {self.instance.network_id}"
        )
    ...
    def __get_container_limits_config(self, limits: RessourceLimits):
        ...
        config["memswap_limit"] = current_app.config[
            "INSTANCE_CONTAINER_MEM_PLUS_SWAP_LIMIT"
        ]
        config["kernel_memory"] = current_app.config[
            "INSTANCE_CONTAINER_MEM_KERNEL_LIMIT"
        ]
    ...
    def mount(self):
        ...
        if exercise_entry_service.persistance_container_path:
            ...
            mounts = {
                self.dc.local_path_to_host(instance_entry_service.overlay_merged): {
                    "bind": "/home/user",
                    "mode": "rw",
                }
            }
    ...
    def __start_peripheral_services(self, exercise: Exercise, entry_container):
        ...
        internet_services = [
            service for service in services if service.exercise_service.allow_internet
        ]
        ...
        network_name += f"{self.instance.exercise.short_name}"
        network_name += (
            f"-v{self.instance.exercise.version}-peripheral-to-entry-{self.instance.id}"
        )
        ...
        for service in services:
            container_name = (
                f"{DOCKER_RESSOURCE_PREFIX}{self.instance.exercise.short_name}"
            )
    ...
    def start(self):
        ...
        assert self.is_mounted(), (
            "Instances should always be mounted, except just before they are removed"
        )
        ...
        entry_to_ssh_network = self.dc.create_network(
            name=entry_to_ssh_network_name,
            internal=not self.instance.exercise.entry_service.allow_internet,
        )
        ...
        default_config = self.__get_container_config_defaults()
        ressource_limit_config = self.__get_container_limits_config(
            exercise.entry_service.ressource_limit
        )
        ...
        if exercise.entry_service.disable_aslr:
            container_setup_script += (
                "touch /etc/aslr_disabled && chmod 400 /etc/aslr_disabled\n"
            )

        if self.instance.submission:
            container_setup_script += "touch /etc/is_submission\n"

        self.dc.container_add_file(
            container, "/tmp/setup.sh", container_setup_script.encode("utf-8")
        )
        ret = container.exec_run('bash -c "/tmp/setup.sh"')
        if ret.exit_code != 0:
    ...
    def stop(self):
        ...
        try:
            ...
            log.error("Failed to stop networking", exc_info=True)
    ...
    def is_running(self):
        ...
        if any(
            map(
                lambda e: e.exercise_service.allow_internet,
                self.instance.peripheral_services,
            )
        ):
            if not self.dc.network(
                self.instance.peripheral_services_internet_network_id
            ):
                return False
    ...
    def run_tests(self):
        ...
        run_test_cmd = "/usr/local/bin/submission_tests"
        ...
        try:
            if os.path.exists(self.instance.persistance_path):
                subprocess.check_call(
                    f"sudo rm -rf {self.instance.persistance_path}", shell=True
                )
        except Exception:
            ...
            if path.is_dir():
                for path in path.glob("*"):
                    if path.parts[-1] in [".ssh"]:
                        ...
                        continue
                    subprocess.check_call(
                        ["/usr/bin/sudo", "/bin/rm", "-rf", "--", path.as_posix()],
                        shell=False,
                    )
        except Exception:
            log.error(
                f"Error during purgeing of persisted data {self.instance}",
                exc_info=True,
            )
            raise
        finally:
