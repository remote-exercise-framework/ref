...
class InstanceService(CommonDbOpsMixin, ModelToStringMixin, db.Model):
    ...
    exercise_service_id: int = db.Column(
        db.Integer,
        db.ForeignKey("exercise_service.id", ondelete="RESTRICT"),
        nullable=False,
    )
    exercise_service: "ExerciseService" = db.relationship(
        "ExerciseService",
    ...
    instance_id: int = db.Column(
        db.Integer,
        db.ForeignKey("exercise_instance.id", ondelete="RESTRICT"),
        nullable=False,
    )
...
class Instance(CommonDbOpsMixin, ModelToStringMixin, db.Model):
    ...
    entry_service: InstanceEntryService = db.relationship(
        "InstanceEntryService",
        uselist=False,
        back_populates="instance",
        passive_deletes="all",
    )
    peripheral_services: List[InstanceService] = db.relationship(
    ...
    peripheral_services_internet_network_id: str = db.Column(
        db.Text(), nullable=True, unique=True
    )
    peripheral_services_network_id: str = db.Column(
        db.Text(), nullable=True, unique=True
    )
    ...
    submission: "Submission" = db.relationship(
        "Submission",
...
class SubmissionTestResult(CommonDbOpsMixin, ModelToStringMixin, db.Model):
    ...
    def __init__(
        self, task_name: str, output: str, success: bool, score: ty.Optional[float]
    ) -> None:
        super().__init__()
...
class Submission(CommonDbOpsMixin, ModelToStringMixin, db.Model):
    ...
    origin_instance_id: int = db.Column(
        db.Integer,
        db.ForeignKey("exercise_instance.id", ondelete="RESTRICT"),
        nullable=False,
    )
    origin_instance: Instance = db.relationship(
    ...
    """
    submitted_instance_id: int = db.Column(
        db.Integer,
        db.ForeignKey("exercise_instance.id", ondelete="RESTRICT"),
        nullable=False,
    )
    submitted_instance: Instance = db.relationship(
    ...
    grading_id: ty.Optional[int] = db.Column(
        db.Integer, db.ForeignKey("grading.id", ondelete="RESTRICT"), nullable=True
    )
    ...
    submission_test_results: List[SubmissionTestResult] = db.relationship(
    ...
    extended_submission_test_results: List[SubmissionExtendedTestResult] = (
        db.relationship(
...
class Grading(CommonDbOpsMixin, ModelToStringMixin, db.Model):
    ...
    submission: List[Submission] = db.relationship(
        "Submission",
    ...
    last_edited_by_id: int = db.Column(
