...
from ref.core import InconsistentStateError, InstanceManager, admin_required, flash
...
def instances_view_by_exercise(exercise_name):
    try:
        ...
        flash.error("Invalid exercise name")
        abort(400)
    ...
    if exercise_version:
        try:
            ...
            flash.error("Invalid exercise version")
            abort(400)
...
def instance_view_submissions(instance_id):
    ...
    return _instances_render_view(
        instances, title=f"Submissions of instance {instance.id}"
    )
...
def instances_view_all():
    instances = (
        Instance.query.options(joinedload(Instance.exercise), joinedload(Instance.user))
        .filter(Instance.submission == None)  # noqa: E711
        .all()
    )
...
def instance_delete(instance_id):
    ...
    if not SystemSettingsManager.SUBMISSION_ALLOW_DELETE.value:
        if instance.submissions:
            flash.error(
                f"Unable to delete instance {instance_id}, since it has associated submissions."
            )
            return redirect_to_next()
        elif instance.submission:
            flash.error(
                f"Unable to delete instance {instance_id}, since submission deletion is disabled."
            )
            return redirect_to_next()
...
def instance_review(instance_id):
    ...
    return render_template(
        "instances_review.html",
        title=title,
        file_browser_path=instance_directory,
        instance=instance,
    )
