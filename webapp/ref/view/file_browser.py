...
def _get_file_list(dir_path, base_dir_path, list_hidden_files=False):
    ...
    if dir_path.strip("/") != base_dir_path.strip("/"):
        ...
        files.append({"path": relative_path, "is_file": False})
    ...
    for path in Path(dir_path).glob("*"):
        ...
        files.append({"path": relative_path, "is_file": is_file})

    if not list_hidden_files:
        files = [
            f
            for f in files
            if not Path(f["path"]).parts[-1].startswith(".") or f["path"].endswith("..")
        ]
...
def file_browser_processor():
    ...
    return dict(file_browser_sign_path=sign_path, file_browser_ls=list_dir)
...
def file_browser_load_file():
    ...
    if not final_path.as_posix().startswith(path_prefix.as_posix()):
        log.warning(
            f"Given path ({final_path}) points outside of the signed prefix ({path_prefix})"
        )
        return abort(400)
    ...
    if final_path.is_file():
        ...
        response = {"type": "file", "content": content, "extension": file_extension}
        ...
        files = _get_file_list(
            ...
            list_hidden_files=not hide_hidden_files,
        )
        ...
        response = {
            ...
            "content": render_template(
                "file_browser/file_tree.html", files=files, file_load_url=file_load_url
            ),
        }
...
def file_browser_save_file():
    rendered_alert = render_template(
        "file_browser/alert.html", error_message="Saving is currently not supported!"
    )
