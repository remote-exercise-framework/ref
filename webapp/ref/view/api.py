...
def start_and_return_instance(instance: Instance):
    ...
    if not ExerciseImageManager(instance.exercise).is_build():
        log.error(f'User {instance.user} has an instance ({instance}) of an exercise that is not build. Possibly someone delete the docker image?')
    ...
    if not running:
        log.info('Instance is not running, starting...')
