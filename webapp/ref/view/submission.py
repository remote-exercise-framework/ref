...
from ref.core import InstanceManager, admin_required, flash
from ref.core.logging import get_logger
from ref.core.util import redirect_to_next
from ref.model import Instance, Submission, SystemSettingsManager, User
...
def submissions_view_all():
    ...
    return render_template(
        "submissions_view_all.html", title="", submissions=submissions
    )
...
def submission_delete(submission_id):
    ...
    instance = Instance.query.filter(
        Instance.id == submission.submitted_instance_id
    ).one_or_none()
...
def submissions_by_instance(instance_id):
    submissions = Submission.query.filter(
        Submission.origin_instance_id == instance_id
    ).all()
    submissions = sorted(submissions, key=lambda e: e.submission_ts, reverse=True)

    return render_template(
        "submissions_view_all.html",
        title=f"Submissions of instance {instance_id}",
...
def submissions_by_user(user_id):
    ...
    submissions: typing.List[Submission] = [
        instance.submission for instance in user.submissions
    ]
    submissions = sorted(submissions, key=lambda e: e.submission_ts, reverse=True)

    return render_template(
        "submissions_view_all.html",
        title=f"Submissions of user {user_id}",
        submissions=submissions,
    )
