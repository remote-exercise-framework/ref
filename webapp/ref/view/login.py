...
class LoginForm(Form):
    username = StringField(
        ...
        validators=[validators.DataRequired(), validators.Regexp(r"[0-9]+")],
        default="",
    )
    ...
    if form.submit.data and form.validate():
        ...
        if not user:
            ...
            form.password.errors += [
                "Please note that this login is not supposed to be used by students."
            ]
        ...
        if (
            ...
            or (not user.is_admin and not user.is_grading_assistant)
        ):
            form.password.errors += ["Invalid username or password"]
            form.password.errors += [
                "Please note that this login is not supposed to be used by students."
